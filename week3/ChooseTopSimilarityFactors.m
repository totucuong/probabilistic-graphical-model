...
% Your code here:
similarityScores = getAllSimilarityScores(allFactors);
[sortedScores I] = sort(similarityScores, 'descend');

factors = allFactors(I(1:F));
end


function sscore = GetSimilarityScore(factor)
    sscore = factor.val(1);
end

function similarityScores = getAllSimilarityScores(allFactors)
    similarityScores = [];
    for i=1:length(allFactors)
        similarityScores = [similarityScores GetSimilarityScore(allFactors(i))];
    end
end
