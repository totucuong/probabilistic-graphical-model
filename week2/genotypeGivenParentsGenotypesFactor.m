...
for i=1:m % genotype of parent 2
    allelesParentTwo = genotypesToAlleles(i,:);
    for j=1:m % genotype of parent 1
        allelesParentOne = genotypesToAlleles(j,:);
        childGenotypes = allelesToGenotypes(allelesParentOne, allelesParentTwo);
        tbl = tabulate(childGenotypes(:));
        assignments = [tbl(:,1), repmat(j, size(tbl,1),1), repmat(i, size(tbl,1),1)];
        values = tbl(:,3) ./100;
        genotypeFactor = SetValueOfAssignment(genotypeFactor, assignments, values);
    end
end

% Solution 2 not working at the moment
% assignments = IndexToAssignment(1:prod(genotypeFactor.card), genotypeFactor.card);
% for i=1:size(assignments,1)
%     genotypeParent1 = assignments(i,2);
%     allelesParentOne = genotypesToAlleles(genotypeParent1,:);
%     genotypeParent2 = assignments(i,3);
%     allelesParentTwo = genotypesToAlleles(genotypeParent2,:);
%     childGenotypes = allelesToGenotypes(allelesParentOne, allelesParentTwo);
%     tbl = tabulate(childGenotypes(:));
%     assign = [tbl(:,1), repmat(genotypeParent1, size(tbl,1),1), repmat(genotypeParent1, size(tbl,1),1)];
%     values = tbl(:,3) ./100;
%     genotypeFactor = SetValueOfAssignment(genotypeFactor, assign, values);
% end
