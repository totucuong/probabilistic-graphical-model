...
assigments = IndexToAssignment(1:prod(phenotypeFactor.card), phenotypeFactor.card);

for i = 1:size(assigments,1)   
    if (assigments(i,end) == 1) || (assigments(i,end) == 2) % AA or Aa
        if assigments(i,1) == 1
            phenotypeFactor = SetValueOfAssignment(phenotypeFactor, assigments(i,:), double(isDominant));
        else
            phenotypeFactor = SetValueOfAssignment(phenotypeFactor, assigments(i,:), double(~isDominant));
        end
    else
