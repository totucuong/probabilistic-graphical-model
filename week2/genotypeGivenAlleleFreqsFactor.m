...
%     alleleIds = genotypesToAlleles(i,:);
%     genotypeFactor.val(i) = prod(alleleFreqs(alleleIds));
%     if alleleIds(1) ~= alleleIds(2)
%         genotypeFactor.val(i) = genotypeFactor.val(i)*2;
%     end
% end

% Solution 2
factor = ones(genotypeFactor.card,1) + (genotypesToAlleles(:,1) ~= genotypesToAlleles(:,2));
genotypeFactor.val = (factor .* prod(alleleFreqs(genotypesToAlleles),2))';
